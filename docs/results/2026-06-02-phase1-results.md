...
6. **LAST_UNITS 처리량 수치의 의미**: LAST_UNITS는 재고 10개가 라운드 1에서 소진되고, 나머지 측정 라운드(2~5)는 200건씩 전부 sold-out 처리된다. throughput은 `성공 건수 / 전체 측정 wall-clock`로 계산되므로, 이 값은 "이 전략이 LAST_UNITS 수요를 얼마나 빨리 처리하는가"가 아니라 "5라운드 전체 워크로드를 도는 데 걸린 시간"에 가깝다. 즉 LAST_UNITS의 throughput 컬럼은 단독으로 읽으면 오해의 소지가 있다 — 이 시나리오에서 의미 있는 결론은 **정합성(oversell)**이지 처리량이 아니다.
